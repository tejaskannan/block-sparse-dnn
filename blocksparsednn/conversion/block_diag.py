...
from blocksparsednn.utils.tf_utils import create_diagonal_pattern
...
def convert_block_sparse_network(weights: Dict[str, np.ndarray],
    ...
    for i, (group_name, group_vars) in enumerate(block_groups.items()):
        ...
        pattern = create_diagonal_pattern(input_dim=int(input_size / block_size),
                                          output_dim=int(output_size / block_size))

        # Extract the rows and columns from the block size
        rows: List[int] = []
        cols: List[int] = []
        ...
        # Create the random connection indices
        random_weights = weights['{0}/random-conn:0'.format(group_name)]
        random_weights_var = convert_matrix(name='{0}_RANDOM_CONN'.format(group_name),
                                            mat=random_weights,
                                            precision=PRECISION,
                                            width=WIDTH,
                                            is_msp=is_msp)

        components.append(random_weights_var)

        random_idx = metadata['{0}/random-conn-idx'.format(group_name)]
        random_idx_name = '{0}_RANDOM_IDX'.format(convert_name(group_name))
        random_idx_var = 'static uint16_t {0}[] = {{{1}}};'.format(random_idx_name, ','.join(map(str, random_idx)))

        components.append(random_idx_var)
