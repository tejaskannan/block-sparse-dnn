...
def convert_block_layer(layer_name: str,
    ...
    for block_idx, block in enumerate(blocks):
        block_var = '{0}_{1}'.format(layer_name, block_idx)
...
def convert_block_sparse_network(weights: Dict[str, np.ndarray],
    ...
    for i in range(len(hypers['hidden_units'])):
        ...
        pattern = np.ones(shape=(int(input_size / block_size), int(output_size / block_size)))

        # Extract the rows and columns, as well as the blocks
        rows: List[int] = []
        cols: List[int] = []
        blocks: List[np.ndarray] = []

        kernel_name = '{0}/kernel:0'.format(layer_name)
        kernel = weights[kernel_name]
        ...
        for row in range(pattern.shape[0]):
            for col in range(pattern.shape[1]):
                if (pattern[row, col] == 1):
                    ...
                    block = kernel[row_start:row_end, col_start:col_end]
                    blocks.append(block) 

        bias = weights['{0}/bias:0'.format(layer_name)]

        # Create the block matrices
        block_declaration = convert_block_layer(layer_name=layer_name,
                                                blocks=blocks,
                                                bias=bias,
